...
export class StrimService {
  private cache: CacheManager<Buffer>

  constructor() {
    this.cache = new CacheManager(30)
  }

async writeStrim({ file, pathDir, fileName }: { pathDir: string; file: Buffer, fileName: string}): Promise<string> {
    try {
 ...
 async getFoto(req: http.IncomingMessage, res: http.ServerResponse<http.IncomingMessage>) { 
  if (!req.url) return;
  ...
  try {
      ...
      /** example query: ?w=200&h=200&blur=1&rotate=20&linear=1.2,1.8&modulate=1.5,1.8&border=2,red */

       const linerParams = linear ? Array.from((linear as string).split(',') ).map(value => Number(value)) : []
       const modulateParams = modulate ? Array.from((modulate as string).split(',') ).map(value => Number(value)) : []
       const borderParams = border ? Array.from((border as string).split(',') ) : []
